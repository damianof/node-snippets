...
module.exports = (function(){
		...
		try {
			...
			var initConnection = function(){
				...
				if (rabbitConfig.useCluster){
					...
					logger.debug('--- Using RabbitMQ [Cluster] ---', amqpOptions.hosts);
				} else {
					...
					logger.debug('--- Using RabbitMQ Instance at ' + rabbitConfig.rabbit.host + ':' + rabbitConfig.rabbit.port + ' ---');
				}
				
				logger.debug('--- NMAPI-Queueing: setupRabbitMQ: amqpOptions', JSON.stringify(amqpOptions));
				...
				connection.on('ready', function() {
					logger.debug('--- Connection with the RabbitMQ server established ---');
	...
	*/
	var assertExchange = function(exchangeKey, exchangeOptions, callback) {
		logger.debug('MAPI-Queueing: assertExchange [' + exchangeKey + ']', exchangeOptions);
		...
		try {
			connection.exchange(exchangeKey
			 	...
			 	, function(result) {
			 		...
			 		if (callback){
			 			logger.debug('NMAPI-Queueing: assertExchange [' + exchangeKey + ']');
			 			callback(result);
			 		}
			});
		} catch(e){
	...
	var assertQueue = function(queueKey, queueOptions, callback) {
		logger.debug('MAPI-Queueing: assertQueue [' + queueKey + ']', queueOptions);
		...
		try {
			connection.queue(queueKey
				, queueOptions
				, function(result) {
					logger.debug('NMAPI-Queueing: assertQueue [' + queueKey + ']');
			...
			logger.error('NMAPI-Queueing: assertQueue [' + queueKey + '] ERROR', e);
			throw e;
		}
	};
	
	var bindQueueToExchange = function(queue, exchangeKey, callback) {
		logger.debug('NMAPI-Queueing: bindQueueToExchange [' + queue.name + '] to exchange [' + exchangeKey + ']');
		...
		try {
			queue.bind(exchangeKey
				...
				queue.on('queueBindOk', function(){
					logger.cyan('queueBindOk for queue: ' + queue.name);
			...
			logger.error('NMAPI-Queueing: bindQueueToExchange [' + queue.name + '] ERROR', e);
			throw e;
		}
	};
	...
	var initJobQueues = function(numJobQueues, jobQueuesConsumeCallback, callback){
		...
		if (!exchanges.jobExchange){
			...
			var setupJobQueues = function(){
				logger.debug('MAPI-Queueing: initJobQueues: numJobQueues:', numJobQueues);

				var queuesCount = 0;
				var onBindJobQueue = function (jobQueue){
					logger.debug('MAPI-Queueing: onBindJobQueue: result:', jobQueue.name);
					queuesCount++;
					...
					if (jobQueuesConsumeCallback){
						...
						logger.debug('MAPI-Queueing: onBindJobQueue: subscribe to: ', jobQueue.name);
						jobQueue.subscribe({
								ack: true
								, prefetchCount: 1
							},
					...
					if (queuesCount != numJobQueues){
						...
						logger.debug('MAPI-Queueing: onBindJobQueue: all ' + queuesCount + ' job queues ready.');
						callback({
				...
				var onAssertJobQueue = function(jobQueue) {
					logger.debug('MAPI-Queueing: onAssertJobQueue:', jobQueue.name);
