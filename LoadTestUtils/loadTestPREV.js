...
var task_StartSession = {
	...
	, exec: function(cb){
		...
		client.post(apiRoutes.sessions, createSessionPayload, function(err, req, res, obj){
			...
			cb({what:'StartSession: result', err: err, res: res});
		});
	}
};
...
var task_LoadMetadata = {
	...
	, exec: function(cb){
		...
		client.put(apiRoutes.sessionid, loadMetadataPayload, function(err2, req2, res2, obj2){
			...
			cb({what:'LoadMetadata: result', err: err2, res: res2});
		});
	}	
};
...
var TaskSendID3 = function(sequence, payload){
	...
	self.exec = function(cb){
		...
		client.put(apiRoutes.sessionid, payload, function(err, req, res, obj){
			cb({what:'SendID3 ' + self.sequence + ': result', err: err, res: res});
		});
	};
};
...
var task_StopEvent = {
	...
	, exec: function(cb){
		...
		client.put(apiRoutes.sessionid, data, function(err, req, res, obj){
			...
			cb({what:'StopEvent: result', err: err, res: res});
		});
	}
};
...
var task_EndSession = {
	...
	, exec: function(cb){
		...
		client.del(apiRoutes.sessionid, function(err, req, res, obj){
			client.close();
			cb({what:'EndSession: result', err: err, res: res});
		});
	}
};

// helper
var printTaskResult = function(args){										
	if (args.err){
...
queue.push(task_StartSession, 1, function (args) {
	printTaskResult(args);
...
// add some items to the queue 
queue.push(task_LoadMetadata, 2, function (args) {
    printTaskResult(args);
});

var sendID3_Callback = function (args) {
    printTaskResult(args);
};
...
for (var i = 0; i < _howManyId3; i++){
	...
	queue.push(task_SendID3, sequence, sendID3_Callback);
}
// add some items to the queue
queue.push(task_StopEvent, ++sequence, function (args) {
    printTaskResult(args);
});
// add some items to the queue 
queue.push(task_EndSession, ++sequence, function (args) {
    printTaskResult(args);
});
